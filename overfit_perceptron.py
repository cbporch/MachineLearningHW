...
def f(x, c):
    c[0] = 0
    return np.sign(sum(legendre.legval(x=x, c=c)))
    ...
    for num in N:                               # 100
        ...
        for dim in Q_f:  # 20
            ...
            for count in range(experiment_size):  # = 8,000,000 runs
                    ...
                    g2_sq_err += sum((np.sign(polyval(test_x[i][1], g2_coeff)) - test_y[i]) ** 2 for i in range(k)) / k

                    # calculate squared error for g10
                    g10_sq_err += sum((np.sign(polyval(test_x[i][1], g10_coeff)) - test_y[i]) ** 2 for i in range(k)) / k
            ...
            if overfit < lo:
                lo = overfit
                print("\t\tfor sigma: {0:1.2f} -> Lo: H10 - H2: {1:.4f}".format(1, overfit))
            elif overfit > hi:
                hi = overfit
                print("\t\tfor sigma: {0:1.2f} -> Hi: H10 - H2: {1:.4f}".format(1, overfit))
