...
sigma_squared = np.arange(0, 2, 0.05)
experiment_size = 100
...
def g_10(x, y):
    return polyfit(x, y, 10)


def run_experiments():
    lo = hi = 1
    exper_range = range(experiment_size)
    # for num in N:                               # 100
    num = 20
    y = np.zeros(num)
    ...
    for dim in Q_f:  # 20
        ...
        for sig in sigma_squared:  # 40
            ...
            for count in exper_range:  # = 8,000,000 runs
                ...
                g2_sq_err += sum((polyval(test_x[i], g2_coeff) - test_y[i]) ** 2 for i in range(k)) / k

                # calculate squared error for g10
                g10_sq_err += sum((polyval(test_x[i], g10_coeff) - test_y[i]) ** 2 for i in range(k)) / k
